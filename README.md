...
```swift
.package(url: "https://github.com/palgroup/palbackend-ios", from: "0.31.0")
```

Add the `Palbe` library to your app target — that is the entire wiring. There is
no build-tool plugin to attach and no build phase to configure: the typed client
is generated by the Palbase CLI and committed to your repo (see below).
