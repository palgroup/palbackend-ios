// swift-tools-version:6.0
import PackageDescription

// palbackend-ios — the managed-backend SDK (binary distribution).
//
// `import Palbe` gives a single entry point `pb`: generated typed endpoint
// calls (`pb.rooms.create(...)`), the untyped escape hatch (`pb.call`/
// `pb.upload`), and auth (`pb.auth.*`). The SDK ships as a closed-source
// XCFramework — `Palbe` is a binaryTarget, so consumers see only the public
// API (Cmd+Click shows no implementation). Source lives in a private repo.
//
// Typed calls are generated by the `palbase` CLI (`palbase backend types
// --lang swift`) from an Xcode Run Script build phase — see README. There is
// no SwiftPM codegen plugin: one path, the CLI.
let package = Package(
    name: "Palbe",
    platforms: [
        .iOS(.v18),
        .macOS(.v15),
    ],
    products: [
        .library(name: "Palbe", targets: ["Palbe"]),
        .library(name: "PalbeMessaging", targets: ["Palbe", "PalbeMessaging"]),
        .library(name: "PalbeCall", targets: ["Palbe", "PalbeMessaging", "PalbeCall", "LiveKitWebRTC", "RustLiveKitUniFFI"]),
        .library(name: "PalbePurchases", targets: ["PalbePurchases"]),
    ],
    targets: [
        .binaryTarget(
            name: "Palbe",
            url: "https://github.com/palgroup/palbackend-ios/releases/download/v0.58.4/Palbe.xcframework.zip",
            checksum: "9ceac5d023adf171b9900677000ac20393d49aaaffab41e976804ac0363f27d4"
        ),
        .binaryTarget(
            name: "PalbeMessaging",
            url: "https://github.com/palgroup/palbackend-ios/releases/download/v0.58.4/PalbeMessaging.xcframework.zip",
            checksum: "48ffb56872612846955a07418a2299bd1cce3c85a89fbc352475888b50022098"
        ),
        .binaryTarget(
            name: "PalbeCall",
            url: "https://github.com/palgroup/palbackend-ios/releases/download/v0.58.4/PalbeCall.xcframework.zip",
            checksum: "6e68a03d31621c5871237bc106f411cbf1e797fbd78638ab724a283f1f316c8c"
        ),
        .binaryTarget(
            name: "PalbePurchases",
            url: "https://github.com/palgroup/palbackend-ios/releases/download/v0.58.4/PalbePurchases.xcframework.zip",
            checksum: "4d9cf9ef8ff48cc2d0eef099f693a949f90f562036f856f52a166beb9a0fa9d3"
        ),
        .binaryTarget(
            name: "RustLiveKitUniFFI",
            url: "https://github.com/livekit/livekit-uniffi-xcframework/releases/download/0.0.6/RustLiveKitUniFFI.xcframework.zip",
            checksum: "0d3f2ce159a224c728f8b131068d53bbf9b13d968cda0edc68a6a2290f2651ed"
        ),
        .binaryTarget(
            name: "LiveKitWebRTC",
            url: "https://github.com/livekit/webrtc-xcframework/releases/download/144.7559.11/LiveKitWebRTC.xcframework.zip",
            checksum: "07c5caf718058af3c528dcabd257298c40e5a8527e4fb9f47c48336ba5899853"
        ),
        .executableTarget(name: "palbase-swiftgen", path: "Sources/palbase-swiftgen"),
    ],
    swiftLanguageModes: [.v6]
)
